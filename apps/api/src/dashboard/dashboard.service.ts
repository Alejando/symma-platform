...
export class DashboardService {
  ...
  async getStats(therapistId: string) {
    ...
    const [activePatientsCount, atRiskPatients, recentSessions] = await Promise.all([
    ...
    const atRiskDetails = await Promise.all(atRiskPatients.map(async (p) => {
      ...
      let daysInactive = 0;
      if (lastSession) {
        daysInactive = Math.floor((now.getTime() - lastSession.date.getTime()) / (1000 * 3600 * 24));
      } else {
        ...
        if (firstRoutine) {
          daysInactive = Math.floor((now.getTime() - firstRoutine.startDate.getTime()) / (1000 * 3600 * 24));
        }
      }
    ...
    const avgEfficacy = scores.length > 0 ? (scores.reduce((a, b) => a + b, 0) / scores.length) : 0;
